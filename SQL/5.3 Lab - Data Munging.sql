...
WITH commonTableExpression  -- specify the name of the result set we will query
AS                  
...
--TODO
select * from purchaseEvents sort by purchases desc limit 1;
...
select date_format(to_date(eventDate, "MM/dd/yy"),"E") day, sum(purchases) as total_purchases
from purchaseEvents
group by day;
...
--TODO
select to_date(eventDate, "MM/dd/yy") day, avg(purchases) as avg_purchases
from purchaseEvents
group by day;
...
--TODO
select purchases, city, state, email, user_id
from purchaseEvents join emailTable
where email is not null and
emailTable.user_id = purchaseEvents.userId;
