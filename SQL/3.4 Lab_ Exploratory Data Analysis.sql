...
select stockCode, quantity, unitPrice, countryName, round((quantity*unitPrice),2) as totalAmount
...
select * from outdoorProducts where countryName = 'United States';

-- COMMAND ----------

-- TODO
select distinct(countryName) as countryName from outdoorProducts;
...
--TODO
select totalQuantity, alpha3code from
salesQuants join countryCodes on
countryName = EnglishShortName;
...
-- TODO
select count(distinct(EnglishShortName)) from countryCodes;

-- COMMAND ----------

-- TODO
select count(distinct(countryName)) from outdoorProducts;
...
-- TODO
select count(distinct(EnglishShortName)) from modCountryCodes;
